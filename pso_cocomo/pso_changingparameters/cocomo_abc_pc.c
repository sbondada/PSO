...
/* This program does the cost estimation for software projects by making use of the cocomo model :
    effort=a*(size)^b+c*(M.E). the program uses particle swarm optimization technique for estimating the values
    of a ,b and c.
...
double gbesta,gbestb,gbestc,nop,noi,w,c1,c2;
double va[25],vb[25],sa[25],sb[25],pbesta[25],pbestb[25],pbestc[25],eval[25],size[25],f[25],me[20],sc[20],vc[20],gfunc=99999999;
...
void updatec(int k)
 {    
   ...
   z: 
        ...
        if(sk2<-1 || sk2>1) /*restricting cvalue to (-1,1)*/
         {goto z;}
