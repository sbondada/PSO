...
/* This program does the cost estimation for software projects by making use of the cocomo model :
    effort=a*(size)^b. the program uses particle swarm optimization technique for estimating the values
    of a and b. 
    This model employs parameter updation of c1 (local weight)and c2(global weight) values.
 */
...
double gbesta,gbestb,nop=20,noi,w,c1,c2;
double va[25],vb[25],sa[25],sb[25],pbesta[25],pbestb[25],eval[25],size[25],f[25],gfunc=99999999;
...
/* fn(a,b) is the fitness function in this case we have considered mean of  % error as a function*/
...
/* function to update parameters in subsequent iterations*/
...
{  
     ...
     //printf("va\t\t vb\t\t sa\t\t sb\t\tf\t\t pa\t\t pb\t\n");
      ...
      { update_param(i);
           ...
           for(k=0;k<nop;k++)
           {
             printf("%lf \t%lf \t%lf \t%lf \t%lf \t %lf\t %lf\n",va[k],vb[k],sa[k],sb[k],f[k],pbesta[k],pbestb[k]);
    ...
    for(i=0;i<nop;i++)
    {
       gfunc=gbesta*pow(size[i],gbestb);
       printf("%lf \t\t %lf\n",gfunc,eval[i]);
    }
